...
/*----------------------------------文件流-------------------------------------------------------*/
/*
 * 将长度为 len 的内容 buf 写入到 r中的文件描述符中，根据要同步到磁盘中。
 * 跟新累计写入的字节长度
 *
 * 成功返回 1 ，失败返回 0 。
 * 注：fwrite是写入用户缓冲区
...
size_t saveStreamFileWrite(saveStream *r, const void *buf, size_t len) {
    size_t retval;
    //将长度为len的内容buf写入r对应的文件描述符中
    retval = fwrite(buf,len,1,r->io.file.fp);
    //跟新累计写入的字节长度
    r->io.file.buffered += len;

    // 检查写入的字节数，看是否需要执行自动 fsync
    if (r->io.file.autosync &&
        r->io.file.buffered >= r->io.file.autosync)
    {
        //fflush相当于将fd的内容从用户缓冲区中刷新到fp对应的内核缓冲区中
        fflush(r->io.file.fp);
        //将fd对应内核缓冲区中的内容同步到磁盘
        //fdatasync的功能与fsync类似，不同步文件的描述信息，因此可以减少一次IO写操作。
...
/*
 * 流为文件时所使用的结构
 * 3个函数对应saveStream结构中的3个回调函数
 */
const saveStream saveStreamFileIO = {
    // 读函数
    saveStreamFileRead,
    // 写函数
    saveStreamFileWrite,
    ...
    NULL,           //校验和计算函数
    0,              //当前校验和
    { { NULL, 0 } } //saveStream中I/O变量
};


/*
 * 每次通过 saveStream 写入 bytes 指定的字节数量时，执行一次自动的 fsync 。
...
void saveStreamSetAutoSync(saveStream *r, off_t bytes) {
    ...
    //判断回调函数是否正确，主要是防止别缓冲区读写套上了文件读写的函数
    assert(r->read == saveStreamFileIO.read);
 ...
 * 3个函数对应saveStream结构中的3个回调函数
 */
static const saveStream saveStreamBuffesaveStream = {
    // 读函数
    saveStreamBufferRead,
    // 写函数
    saveStreamBufferWrite,
    // 偏移量函数
    saveStreamBufferTell,
    NULL,           //校验和计算函数
    0,              //当前校验和
    { { NULL, 0 } } //saveStream中I/O变量
};
...
size_t saveStreamWriteBulkLongLong(saveStream *r, long long l) {
    ...
    // 写入 $llen\r\nlbuf\r\n
    return saveStreamWriteBulkString(r,lbuf,llen);
}
...
size_t saveStreamWriteBulkString(saveStream *r, const char *buf, size_t len) {
    ...
    // 写入 $<count>\r\n
    if ((nwritten = saveStreamWriteBulkCount(r,'$',len)) == 0) return 0;

    // 写入 <payload>
    if (len > 0 && saveStreamWrite(r,buf,len) == 0) return 0;

    // 写入 \r\n
    if (saveStreamWrite(r,"\r\n",2) == 0) return 0;
...
/*
 * 以带 '\r\n' 后缀的形式写入字符串表示的 count 到 saveStream 
 * prefix：前缀
 * $<count>\r\n
 * 成功返回写入的数量，失败返回 0 。
 */
size_t saveStreamWriteBulkCount(saveStream *r, char prefix, int count) {
    ...
    // 例如： *123\r\n
    cbuf[0] = prefix;//获得前缀*
    clen = 1+ll2string(cbuf+1,sizeof(cbuf)-1,count);//获得数字
    cbuf[clen++] = '\r';//获得后缀'\r\n'
    cbuf[clen++] = '\n';

    // 写入到r中
    if (saveStreamWrite(r,cbuf,clen) == 0) return 0;
...
int saveStreamWriteBulkObject(saveStream *r, robj *obj) {
    ...
    return saveStreamWriteBulkString(r,obj->ptr,sdslen(obj->ptr));

}



/*-------------------------------公共流API，从saveStream载出----------------------------------------------------*/
