...
int slaveTryPartialResynchronization(int fd) {
    ...
    printf("Send to Master: PSYNC  %s  %s\n",psync_runid,psync_offset);
...
void syncCommand(KVClient *c) {
    ...
    printf("server.masterhost:%s    server.repl_state:%d\n",server.masterhost,server.repl_state);
...
void sendBulkToSlave(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (slave->replpreamble) {
        ...
        if (nwritten == -1) {
            printf("Write error sending RDB preamble to slave: %s\n", strerror(errno));
            freeClient(slave);
            return;
        }
    }
...
void replCommand(KVClient *c)
{
    int buflen;
    char* psync = "*3\r\n$5\r\npsync\r\n$1\r\n?\r\n$2\r\n-1\r\n";
    if ((buflen = write(server.repl_transfer_s,psync,strlen(psync))) <= 0) {
        printf("psync ? -1 exec failed\n");
        return;
    }
    addReply(c,shared.ok);
