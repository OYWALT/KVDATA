...
struct KVDataCommand KVDATACommandTable[] = {
    ...
    {"replall",replCommand,1,7}//REPLALL
};
...
int prepareClientToWrite(KVClient *c) {
    ...
    if (aeCreateFileEvent(server.eventsLoop, c->fd, AE_WRITABLE, sendReplyToClient, c) == AE_ERR) 
        ...
        // (c->replstate == KVDATA_REPL_NONE || c->replstate == KVDATA_REPL_ONLINE) &&
        // aeCreateFileEvent(server.eventsLoop, c->fd, AE_WRITABLE, sendReplyToClient, c) == AE_ERR) 
